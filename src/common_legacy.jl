    ...
    for c in s
        ...
        elseif state == reading
            ...
            elseif c == '}'
                ...
                if (length(char_list) == 1) & (char_list[end] == '=')
                    push!(word_list, join(char_list))
                    #push!(word_list, "=")
                    char_list = Char[]
                    push!(char_list, c)
...
function extract(tree::Tree)
    ...
    if is_map
        ...
        #res_keys = Symbol[]
        #res_values = Any[]
        #for (key, value) in zip(keys(el_list), el_list)
        for (key, value) in el_list
            res[Symbol(key)] = value
            #push!(res_keys, Symbol(key))
            #push!(res_values, value)
        end
        return (;res...) # convert it to a NamedTuple. Dict above is used to remove duplicated keys 
        #return NamedTuple{Tuple(res_keys)}(res_values)
    end
    if length(el_list) > 0
        el_list = convert(Vector{typeof(el_list[1])}, el_list)
    end
    return el_list
end
